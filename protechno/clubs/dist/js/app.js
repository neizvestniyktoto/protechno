    ...
    function filtrationItems() {
        ...
        if (filterButtons) filterButtons.forEach((button => {
            button.addEventListener("click", (function(e) {
                ...
                filterReset();
                if (filterValue != "*") filterItems(filterValue);
            }));
        }));
        function filterReset() {
            const hideItems = document.querySelectorAll("[data-grid-item].hide");
            if (hideItems) hideItems.forEach((item => {
                item.classList.remove("hide");
            }));
        }
        function filterItems(filterValue) {
            const girdItems = document.querySelectorAll("[data-grid-item]");
            if (girdItems) girdItems.forEach((item => {
                const itemValue = item.dataset.gridItem;
                if (itemValue != filterValue) item.classList.add("hide");
            }));
        }
    }
    ...
    class Popup {
        ...
        open(selectorValue) {
            if (bodyLockStatus) {
                ...
                } else this.popupLogging(`Йой, такого попапа нет. Проверьте корректность ввода.`);
            }
        }
        close(selectorValue) {
